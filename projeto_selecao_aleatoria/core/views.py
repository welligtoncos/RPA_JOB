...
from .serializers import RPADockerCreateSerializer, RPADockerHistoricoSerializer, RPADockerSerializer, RPAHistoricoSerializer, ResultadoDownloadSerializer
...
class ResultadoDownloadViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def download_file(self, request, id=None, file_name=None):
        ...
        try:
            ...
            if not content_type:
                # Inferir tipo de conteúdo por extensão
            ...
            http_response['Content-Length'] = response['ContentLength']
            
            # Registrar o download
            logger.info(f"Download do arquivo '{file_name}' (ID Processamento: {id}) pelo usuário {request.user.id}")
